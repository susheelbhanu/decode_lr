...
rule kaiju:
    input:
        os.path.join(RESULTS_DIR, "taxonomy/kaiju/kaiju_summary.tsv")
    output:
...
rule kaiju_classify:
    ...
    output:
        out=os.path.join(RESULTS_DIR, "taxonomy/kaiju/{sid}_kaiju_out.txt"),
        names=os.path.join(RESULTS_DIR, "taxonomy/kaiju/{sid}_kaiju_names.txt")
    conda:
...
rule kaiju_table:
    input:
        expand(os.path.join(RESULTS_DIR, "taxonomy/kaiju/{sid}_kaiju_out.txt"), sid=SAMPLES.index)
    output:
        os.path.join(RESULTS_DIR, "taxonomy/kaiju/kaiju_summary.tsv")
    conda:
        os.path.join(ENV_DIR, "kaiju.yaml")
